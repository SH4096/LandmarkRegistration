...
class LandmarkRegistrationWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    for viewName in self.viewNames:
      self.volumeSelectors[viewName] = cjyx.qDRMLNodeComboBox()
      self.volumeSelectors[viewName].nodeTypes = ( ("vtkDRMLScalarVolumeNode"), "" )
      ...
      self.volumeSelectors[viewName].setDRMLScene( cjyx.drmlScene )
    ...
    self.transformSelector = cjyx.qDRMLNodeComboBox()
    self.transformSelector.nodeTypes = ( ("vtkDRMLTransformNode"), "" )
    ...
    self.transformSelector.setDRMLScene( cjyx.drmlScene )
    ...
    try:
      cjyx.modules.registrationPlugins
    ...
    self.localRefinementMethods = sorted(cjyx.modules.registrationPlugins.keys())
    for localRefinementMethod in self.localRefinementMethods:
      plugin = cjyx.modules.registrationPlugins[localRefinementMethod]
    ...
    self.registrationTypes = sorted(cjyx.modules.registrationPlugins.keys())
    for registrationType in self.registrationTypes:
      plugin = cjyx.modules.registrationPlugins[registrationType]
    ...
    for selector in self.volumeSelectors.values():
      selector.connect("currentNodeChanged(vtkDRMLNode*)", self.onVolumeNodeSelect)
  ...
  def setupDialog(self):
    ...
    if not self.volumeSelectDialog:
      self.volumeSelectDialog = qt.QDialog(cjyx.util.mainWindow())
      ...
      for viewName in ('Fixed', 'Moving',):
        self.volumeDialogSelectors[viewName] = cjyx.qDRMLNodeComboBox()
        self.volumeDialogSelectors[viewName].nodeTypes = ( ("vtkDRMLScalarVolumeNode"), "" )
        ...
        self.volumeDialogSelectors[viewName].setDRMLScene( cjyx.drmlScene )
  ...
  def onVolumeDialogApply(self):
    ...
    if fixedID and movingID:
      ...
      if not transformed:
        ...
        transformed = cjyx.drmlScene.GetFirstNodeByName(transformedName)
        if not transformed:
          transformed = volumesLogic.CloneVolume(cjyx.drmlScene, moving, transformedName)
  ...
  def addObservers(self):
    """Observe the drml scene for changes that we wish to respond to.
    scene observer:
    ...
    """
    tag = cjyx.drmlScene.AddObserver(cjyx.drmlScene.NodeAddedEvent, self.landmarksWidget.requestNodeAddedUpdate)
    self.observerTags.append( (cjyx.drmlScene, tag) )
    tag = cjyx.drmlScene.AddObserver(cjyx.drmlScene.NodeRemovedEvent, self.landmarksWidget.requestNodeAddedUpdate)
    self.observerTags.append( (cjyx.drmlScene, tag) )
  ...
  def overlayFixedOnTransformed(self):
    ...
    if transformedNode:
      compositeNodes = cjyx.util.getNodesByClass('vtkDRMLSliceCompositeNode')
  ...
  def onRegistrationType(self,pickedRegistrationType):
    ...
    interfaceClass = cjyx.modules.registrationPlugins[pickedRegistrationType]
  ...
  def onLocalRefinementMethod(self,pickedLocalRefinementMethod):
    ...
    interfaceClass = cjyx.modules.registrationPlugins[pickedLocalRefinementMethod]
  ...
  def restrictLandmarksToViews(self):
    ...
    """
    cjyx.drmlScene.StartState(cjyx.drmlScene.BatchProcessState)
    volumeNodes = self.currentVolumeNodes()
    if self.sliceNodesByViewName:
      landmarks = self.logic.landmarksForVolumes(volumeNodes)
      activePointLists = []
      ...
      allPointLists = cjyx.util.getNodesByClass('vtkDRMLMarkupsFiducialNode')
    ...
    cjyx.drmlScene.EndState(cjyx.drmlScene.BatchProcessState)
  ...
  def onReload(self,moduleName="LandmarkRegistration"):
    ...
    filePath = eval('cjyx.modules.%s.path' % moduleName.lower())
    ...
    # re-register themselves with cjyx
    oldPlugins = cjyx.modules.registrationPlugins
    cjyx.modules.registrationPlugins = {}
    for plugin in oldPlugins.values():
      pluginModuleName = plugin.__module__.lower()
      if hasattr(cjyx.modules,pluginModuleName):
        # for a plugin from an extension, need to get the source path
        # from the module
        module = getattr(cjyx.modules,pluginModuleName)
        sourceFile = module.path
      else:
        # for a plugin built with cjyx itself, the file path comes
    ...
    cjyx.util.reloadScriptedModule("LandmarkRegistration")
...
class LandmarkRegistrationLogic(ScriptedLoadableModuleLogic):
  ...
  def __init__(self):
    ...
    self.cropLogic = None
    if hasattr(cjyx.modules, 'cropvolume'):
      self.cropLogic = cjyx.modules.cropvolume.logic()
  ...
  def addPoint(self,name,position=(0,0,0),associatedNode=None):
    ...
    pointList = cjyx.drmlScene.GetFirstNodeByName(listName)
    if not pointList:
      pointListNodeID = markupsLogic.AddNewFiducialNode(listName,cjyx.drmlScene)
      pointList = cjyx.drmlScene.GetNodeByID(pointListNodeID)
    ...
    originalActiveList = cjyx.drmlScene.GetNodeByID(originalActiveListID)
  ...
  def volumePointList(self,volumeNode):
    ...
    listNode = cjyx.drmlScene.GetFirstNodeByName(listName)
    if listNode:
  ...
  def collectAssociatedPoints(self,volumeNodes):
    ...
    pointListsInScene = cjyx.util.getNodesByClass('vtkDRMLMarkupsFiducialNode')
    landmarkPointLists = []
      ...
      if pointList not in landmarkPointLists:
        ...
        for pointIndex in range(pointSize):
          ...
          if associatedID in volumeNodeIDs:
            ...
            volumeNode = cjyx.drmlScene.GetNodeByID(associatedID)
...
class LandmarkRegistrationTest(ScriptedLoadableModuleTest):
  ...
  def moveMouse(self,widget,start=(10,10),end=(10,40),steps=20,modifiers=[]):
    """Send synthetic mouse events to the specified widget (qDRMLSliceWidget or qDRMLThreeDView)
  ...
  def test_LandmarkRegistrationBasic(self):
    ...
    w = cjyx.modules.LandmarkRegistrationWidget
  ...
  def test_LandmarkRegistrationVTKv6Picking(self):
    ...
    dataSource = SampleDataSource(sampleName='fixed', uris='https://github.com/Cjyx/CjyxTestingData/releases/download/SHA256/b757f9c61c1b939f104e5d7861130bb28d90f33267a012eb8bb763a435f29d37', fileNames='fixed.nrrd', nodeNames='fixed')
    fixed = SampleDataLogic().downloadFromSource(dataSource)[0]

    dataSource = SampleDataSource(sampleName='moving', uris='https://github.com/Cjyx/CjyxTestingData/releases/download/SHA256/4d818fbb8deacb27dc7fb16d23ebbf241ad5bffa7dfca8c20e4f3c9dbf90a228', fileNames='moving.nrrd', nodeNames='moving')
    ...
    cjyx.util.clickAndDrag(fixedAxialView,start=center,end=center, steps=0)
    self.delayDisplay(f'Added a landmark, translate to drag at {center} to {offset}', 200)

    cjyx.util.clickAndDrag(fixedAxialView,button='Middle', start=center,end=offset,steps=10)
    self.delayDisplay('dragged to translate', 200)
    cjyx.util.clickAndDrag(fixedAxialView,button='Middle', start=offset,end=center,steps=10)
  ...
  def test_LandmarkRegistrationManyLandmarks(self):
    ...
    # enter picking mode
    w.landmarksWidget.addLandmark()
    cjyx.util.clickAndDrag(fixedAxialView,start=center,end=offset, steps=10)
    self.delayDisplay(f'Added a landmark, translate to drag at {center} to {offset}', 200)
    ...
    for row in range(5):
      for column in range(5):
        ...
        # enter picking mode
        w.landmarksWidget.addLandmark()
        cjyx.util.clickAndDrag(fixedAxialView,start=clickPoint,end=offset, steps=10)
