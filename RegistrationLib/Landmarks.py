...
class LandmarksWidget(pqWidget):
  ...
  def updateLandmarkArray(self):
    ...
    self.addButton.setIcon(qt.QIcon(os.path.join(os.path.dirname(cjyx.modules.landmarkregistration.path), 'Resources/Icons/', "icon_Add.png")))
    ...
    for landmarkName in keys:
      ...
      activeButton.setIcon(qt.QIcon(os.path.join(os.path.dirname(cjyx.modules.landmarkregistration.path), 'Resources/Icons/', "icon_Active.png")))
      ...
      removeButton.setIcon(qt.QIcon(os.path.join(os.path.dirname(cjyx.modules.landmarkregistration.path), 'Resources/Icons/', "icon_Trash.png")))
  ...
  def addLandmarkObservers(self):
    ...
    for pointList in cjyx.util.getNodes('vtkDRMLMarkupsFiducialNode*').values():
  ...
  def addLandmark(self):
    ...
    selectionNode.SetReferenceActivePlaceNodeClassName("vtkDRMLMarkupsFiducialNode")
  ...
  def renameLandmark(self):
    ...
    if self.selectedLandmark in landmarks:
      newName = qt.QInputDialog.getText(
          cjyx.util.mainWindow(), "Rename Landmark",
  ...
  def wrappedNodeAddedUpdate(self):
    try:
      ...
      import traceback
      traceback.print_exc()
      qt.QMessageBox.warning(cjyx.util.mainWindow(),
