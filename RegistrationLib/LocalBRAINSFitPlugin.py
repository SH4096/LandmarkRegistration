...
comment = """
  ...
  cjyx LandmarkRegistration module.
...
class LocalBRAINSFitPlugin(RegistrationPlugin):
  ...
  def refineLandmark(self, state):
    ...
    cvpn = cjyx.vtkDRMLCropVolumeParametersNode()
    ...
    roiFixed = cjyx.vtkDRMLAnnotationROINode()
    cjyx.drmlScene.AddNode(roiFixed)
    ...
    croppedFixedVolume = cjyx.drmlScene.GetNodeByID( cvpn.GetOutputVolumeNodeID() )
    fixedVolume.SetAndObserveDisplayNodeID(fixedDisplayNode.GetID())

    # define an roi for the moving
    if timing: roi2Start = time.time()
    roiMoving = cjyx.vtkDRMLAnnotationROINode()
    cjyx.drmlScene.AddNode(roiMoving)
    ...
    croppedMovingVolume = cjyx.drmlScene.GetNodeByID( cvpn.GetOutputVolumeNodeID() )
    ...
    #
    transform = cjyx.vtkDRMLLinearTransformNode()
    cjyx.drmlScene.AddNode(transform)
    ...
    cjyx.cli.run(cjyx.modules.brainsfit, None, parameters, wait_for_completion=True)
    ...
    cjyx.drmlScene.RemoveNode(croppedFixedVolume)
    cjyx.drmlScene.RemoveNode(croppedMovingVolume)
    cjyx.drmlScene.RemoveNode(roiFixed)
    cjyx.drmlScene.RemoveNode(roiMoving)
    cjyx.drmlScene.RemoveNode(transform)
    roiFixed = None
    roiMoving = None
    transform = None
    matrix = None
...
try:
  ...
  cjyx.modules.registrationPlugins = {}
cjyx.modules.registrationPlugins['LocalBRAINSFit'] = LocalBRAINSFitPlugin
